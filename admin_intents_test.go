...
func TestAdminIntentsUseV4DialogProtocol(t *testing.T) {
	...
	pagesSection := text[strings.Index(text, "\npages:\n")+len("\npages:\n"):]
	if got := strings.Count("\n"+pagesSection, "\n  - key:"); got != 2 {
		t.Fatalf("可导航页面数量 = %d，期望 2", got)
	}
	if got := strings.Count(text, "type: open-dialog"); got != 10 {
		t.Fatalf("open-dialog 声明数量 = %d，期望 10", got)
	}
	...
	for _, actionKey := range []string{
		"role.list.action.create",
		"role.list.action.detail",
		"role.list.action.edit",
		"role.list.action.permissions",
		"role.permission_list.action.create",
	} {
